...
namespace Authentication.Controllers
{
    ...
    public class MissionController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("UpdateMissiondata")]
        public async Task<IActionResult> UpdateMissiondata(int missionId, MissionDto change)
        {
            var result = await _missionRepository.UpdateMissiondata(missionId, change);
            try
            {
                if (!ModelState.IsValid)
                {
