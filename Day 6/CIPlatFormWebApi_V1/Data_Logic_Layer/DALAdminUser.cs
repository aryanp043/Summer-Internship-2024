...
namespace Data_Logic_Layer
{
    public class DALAdminUser
    {
        ...
        public async Task<String> UpdateUser(int userId, User change)
        {
            try
            {
                using (var transaction = await _context.Database.BeginTransactionAsync())
                {
                    try
                    {
                        var existingUser = await _context.User.FirstOrDefaultAsync(x => x.Id == userId);
                        if (existingUser == null)
                        {
                            return "user details do not found !!!";
                        }
                        else
                        {
                            //main update code

                            existingUser.FirstName = change.LastName;
                            existingUser.LastName = change.LastName;
                            existingUser.PhoneNumber = change.PhoneNumber;
                            //existingUser.Uid = change.Id;
                            existingUser.EmailAddress = change.EmailAddress;
                            existingUser.Password = change.Password;
                            existingUser.UserType = change.UserType;
